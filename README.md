...
To check if the user has a token use `client.IsAuthorized()`. If so, `client.GetIdentityAsync(tokenResponse)` could be executed to get the claims-based identity for granting a user to be signed in. Then `client.UserId`, `client.Email`, `client.MPinID` and `client.HashMPinID` will return additional user data.
...
PrerollId allows the identity of the user to be injected in the query to the MIRACL server and save the user from typing it one more time when registering. In order to use the this functionality in your web app, you should have an input where the user to enter it to:
```
...
There are two methods for verification of an identity when registering it to the Platform:
 ...
 The field Verification Method in the Platform _customer_ settings is responsible for setting the verification method type.

## Samples

Here are some sample applications which demonstrate how an integration to the [MIRACL platform](https://trust.miracl.cloud/) could be achieved. Note that it doesn't matter what framework you're using, if it support the [dependencies](https://github.com/miracl/maas-sdk-dotnet-v2/#dependencies), you could use this SDK in your application.

In order to run any of them, the fields `CLIENT_ID` and `CLIENT_SECRET` in the `web.config` file should be filled with your valid credential data from the [MIRACL server](https://trust.miracl.cloud/). `baseUri`, which is passed to the `MiraclClient.GetAuthorizationRequestUrlAsync` method, should be the uri of your web application.
